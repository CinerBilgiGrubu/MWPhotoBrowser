...
@interface MWPhotoBrowser ()<GADBannerViewDelegate>

@property (strong,nonatomic) NSMutableDictionary *adPool;
@property (strong,nonatomic) NSMutableDictionary *adContainerPool;
@property (strong, nonatomic) UIView *addContainerView;
...
- (void)_initialisation {
    ...
    // Defaults
    _adPool = [NSMutableDictionary new];
    _adContainerPool = [NSMutableDictionary new];
...
- (void)viewDidLoad {
    ...
    [tracker send:[[[GAIDictionaryBuilder createScreenView] set:[NSString stringWithFormat:@"Galeri"]
                                                         forKey:kGAIScreenName] build]];
    

    [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"page"
                                                          action:@"Galeri"
                                                           label:_titleText
                                                           value:nil] build]];
    
}
#pragma mark Google DFP Advertisement
-(void)adViewDidReceiveAd:(GADBannerView *)bannerView{
   
        //[self.advertisementIndex setValue:[NSNumber numberWithBool:YES] forKey:[NSString stringWithFormat:@"%li",(long)bannerView.tag]];
        [self.adPool setValue:bannerView forKey:[NSString stringWithFormat:@"%li",(long)bannerView.tag]];
    
    GADRequest *req = [GADRequest new];
    req.contentURL = self.url;
    //[self.tablePhotoNews reloadData];
}

-(void)adView:(GADBannerView *)bannerView didFailToReceiveAdWithError:(GADRequestError *)error{
   // LogDebug(@"Failed to recevied ad  =%@",bannerView.adUnitID);
   
       
        
    //[self.advertisementRequestIndex setValue:[NSNumber numberWithBool:NO] forKey:[NSString stringWithFormat:@"%li",(long)bannerView.tag]];
    //[self.advertisementIndex setValue:[NSNumber numberWithBool:NO] forKey:[NSString stringWithFormat:@"%li",(long)bannerView.tag]];
    
}
-(void)swipeleft:(UISwipeGestureRecognizer*)gestureRecognizer{
    
    [UIView animateWithDuration:0.25 animations:^{
            gestureRecognizer.view.frame = CGRectMake(-[UIScreen mainScreen].bounds.size.width, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64);
        //[self jumpToPageAtIndex:gestureRecognizer.view.tag - 100 animated:YES];
        if (gestureRecognizer.view.tag - 100 < [self numberOfPhotos]) {
            CGRect pageFrame = [self frameForPageAtIndex:gestureRecognizer.view.tag - 99];
            [_pagingScrollView setContentOffset:CGPointMake(pageFrame.origin.x - 20, 0) animated:NO];
...
-(void)swiperight:(UISwipeGestureRecognizer*)gestureRecognizer{
    [UIView animateWithDuration:0.25 animations:^{
        gestureRecognizer.view.frame = CGRectMake([UIScreen mainScreen].bounds.size.width, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64);
         //[self jumpToPageAtIndex:gestureRecognizer.view.tag - 102 animated:YES];
        if (gestureRecognizer.view.tag - 100 < [self numberOfPhotos]) {
            CGRect pageFrame = [self frameForPageAtIndex:gestureRecognizer.view.tag - 101];
            [_pagingScrollView setContentOffset:CGPointMake(pageFrame.origin.x + 20, 0) animated:NO];
            [self updateNavigation];
        }
    }completion:^(BOOL finished) {
...
- (void)performLayout {
    ...
    UIView *addContainerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64)];
    addContainerView.backgroundColor = [UIColor blackColor];
    DFPBannerView *bannerView = [[DFPBannerView alloc] initWithAdSize:kGADAdSizeMediumRectangle];
    bannerView.adUnitID =[ZONE_ID_ARRAY objectAtIndex:0];
    bannerView.rootViewController = self;
    bannerView.frame = CGRectMake([UIScreen mainScreen].bounds.size.width / 2 - 150, ([UIScreen mainScreen].bounds.size.height - 64) / 2 - 125, 300, 250);
    bannerView.tag = 100 + 1;
    bannerView.delegate = self;
    [bannerView loadRequest:[DFPRequest request]];
    [addContainerView addSubview:bannerView];
    [self.adContainerPool setValue:addContainerView forKey:[NSString stringWithFormat:@"%li",(long)bannerView.tag]];
    
    addContainerView.tag = 100 + 1;
    
    UISwipeGestureRecognizer * swipeleft=[[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(swipeleft:)];
    swipeleft.direction=UISwipeGestureRecognizerDirectionLeft;
    [addContainerView addGestureRecognizer:swipeleft];
    
    UISwipeGestureRecognizer * swiperight=[[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(swiperight:)];
    swiperight.direction=UISwipeGestureRecognizerDirectionRight;
    [addContainerView addGestureRecognizer:swiperight];
    
    /*for(int i = 1; i < numberOfPhotos / 3 + 1; i++){
        UIView *addContainerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64)];
        addContainerView.backgroundColor = [UIColor blackColor];
        DFPBannerView *bannerView = [[DFPBannerView alloc] initWithAdSize:kGADAdSizeMediumRectangle];
        bannerView.adUnitID =[ZONE_ID_ARRAY objectAtIndex:i % 3];
        bannerView.rootViewController = self;
        bannerView.frame = CGRectMake([UIScreen mainScreen].bounds.size.width / 2 - 150, ([UIScreen mainScreen].bounds.size.height - 64) / 2 - 125, 300, 250);
        bannerView.tag = 100 + (i * 4);
        bannerView.delegate = self;
        [bannerView loadRequest:[DFPRequest request]];
        [addContainerView addSubview:bannerView];
        [self.adContainerPool setValue:addContainerView forKey:[NSString stringWithFormat:@"%li",(long)bannerView.tag]];
        
        addContainerView.tag = 100 + (i * 4);
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        //previousViewController.navigationItem.backBarButtonItem = [self createBackButton];
        
        UIImage *backImage = [[UIImage imageNamed:@"backArrow"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 13, 0, 13)];
...
-(UIBarButtonItem *)createBackButton {
    ...
    UIImage *backImage = [[UIImage imageNamed:@"backArrow"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 13, 0, 13)];
    UIButton *button = [[UIButton alloc]initWithFrame:CGRectMake(0, 13, 0, 13)];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    //eger ilk resimden acildiysa o_gezio Request gonderecegiz.
    if(_currentPageIndex == 0){
        
        MWPhoto *photo = [self photoAtIndex:_currentPageIndex];
        [self request:photo.o_gezio];
    }
    //title header//
    self.titleHeader = [[UILabel alloc] initWithFrame:CGRectMake(10, 64, self.view.bounds.size.width-20, 44)];
    self.titleHeader.text = self.titleText;
    self.titleHeader.textColor = [UIColor whiteColor];
    self.titleHeader.numberOfLines = 0;
    self.titleHeader.textAlignment = NSTextAlignmentCenter;
    self.titleHeader.lineBreakMode = NSLineBreakByWordWrapping;
    self.titleHeader.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
    self.titleHeader.font = [UIFont systemFontOfSize:17];
    self.titleHeader.opaque = NO;
    self.titleHeader.backgroundColor = [UIColor clearColor];
    self.titleHeader.alpha = 0;
...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    //UIImage *image = [[UIImage imageNamed:@"newsBack"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
   // UIBarButtonItem *button = [[UIBarButtonItem alloc] initWithImage:image style:UIBarButtonItemStylePlain target:self action:@selector(goBack)];
    /*UIBarButtonItem *backBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:[self createButton:@"newsBack" frame:CGRectMake(0, 0, 17.5, 25) target:@"goBack"]];
    self.navigationItem.leftBarButtonItems = @[backBarButtonItem];*/
    
    UIButton *backButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 17.5, 25)];
    [backButton setImage:[UIImage imageNamed:@"newsBack"] forState:UIControlStateNormal];
    [backButton addTarget:self action:@selector(goBack) forControlEvents:UIControlEventTouchUpInside];
    [backButton.widthAnchor constraintEqualToConstant: 17.5].active = YES;
    [backButton.heightAnchor constraintEqualToConstant: 25].active = YES;
...
-(UIButton *)createButton:(NSString *)imgName frame:(CGRect)buttonFrame target:(NSString *)buttonTarget{
    SEL buttonSelector = NSSelectorFromString(buttonTarget);
    UIButton *tmpButton = [[UIButton alloc] initWithFrame:buttonFrame];
    UIImage *tmpImage = [UIImage imageNamed:imgName];
    [tmpButton setBackgroundImage:tmpImage forState:UIControlStateNormal];
    [tmpButton addTarget:self action:buttonSelector forControlEvents:UIControlEventTouchUpInside];
    return tmpButton;
}
...
- (MWCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    ...
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        ...
        if ([photo respondsToSelector:@selector(caption)]) {
            ...
            //captionView = [[MWPhotoHeaderView alloc] initWithLabel:@"deneme"];
        }
    }
...
- (CGRect)frameForPagingScrollView {
    CGRect frame = CGRectMake(0, -64, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height); //self.view.bounds;// [[UIScreen mainScreen] bounds];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_currentPageIndex != previousCurrentPage) {
        ...
        if (index % 4 == 0){
            ...
            addContainerView.backgroundColor = [UIColor blackColor];
            DFPBannerView *bannerView = [[DFPBannerView alloc] initWithAdSize:kGADAdSizeMediumRectangle];
            bannerView.adUnitID =[ZONE_ID_ARRAY objectAtIndex:(index / 3) % 3];
            bannerView.rootViewController = self;
            bannerView.frame = CGRectMake([UIScreen mainScreen].bounds.size.width / 2 - 150, ([UIScreen mainScreen].bounds.size.height - 64) / 2 - 125, 300, 250);
            bannerView.tag = 100 + index + 1;
            bannerView.delegate = self;
            [bannerView loadRequest:[DFPRequest request]];
            [addContainerView addSubview:bannerView];
            [self.adContainerPool setValue:addContainerView forKey:[NSString stringWithFormat:@"%li",(long)bannerView.tag]];
            
            addContainerView.tag = 100 + index + 1;
        ...
        if (index % 4 == 1) {
            ...
            ((UIView *)[_adContainerPool objectForKey:[NSString stringWithFormat:@"%lu", 100 + index ]]).frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - 64);
            
            [self.view addSubview:[_adContainerPool objectForKey:[NSString stringWithFormat:@"%lu", 100 + index]]];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    //  [tracker set:kGAIScreenName value:screenName];
    //[tracker send:[[GAIDictionaryBuilder createScreenView] build]];
   // [tracker send:[[[GAIDictionaryBuilder createScreenView] set:[NSString stringWithFormat:@"%@ Galeri", _titleText]
                                                        // forKey:kGAIScreenName] build]];
}

#pragma mark - o_gezio request
-(void)request:(NSString *)url{
   /* long timeStamp = (long)[[NSDate date] timeIntervalSince1970];
    NSString *timestampURL = [NSString  stringWithFormat:@"%@&ns_t=%li&m4=iphone&idfa=%@",url,timeStamp,[[[ASIdentifierManager sharedManager] advertisingIdentifier] UUIDString]];
    //o_gezio URL Request
    NSURLRequest *o_gezioRequest = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:timestampURL]];
    NSURLConnection *req = [[NSURLConnection alloc] initWithRequest:o_gezioRequest delegate:self];
    [req start];*/
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            self.title = NSLocalizedString(@"Seçilen resimler", nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
                photosText = NSLocalizedString(@"resim", @"Used in the context: '1 photo'");
            } else {
                photosText = NSLocalizedString(@"resim", @"Used in the context: '3 photos'");
            }
    ...
    if ([photo underlyingImage] == nil || ([photo respondsToSelector:@selector(isVideo)] && photo.isVideo)) {
        ...
        _actionButton.tintColor = [UIColor whiteColor]; // Tint to hide button
    } else {
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    [UIView animateWithDuration:animationDuration animations:^(void) {
        ...
        //[self.titleHeader setAlpha:alpha];
        self.titleHeader.alpha = 0;
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
            ...
            if (photo.caption) {
                ...
                [items addObject:[NSURL URLWithString:[defaultUser objectForKey:@"shortUrl"]]];
            }
            ...
            self.activityViewController.completionWithItemsHandler = ^(NSString *activityType, BOOL completed, NSArray *returnedItems, NSError *activityError) {
                ...
                if (completed) {
                    ...
                    if ([activityType isEqualToString:@"ph.telegra.Telegraph.Share"]) {
                        //Telegram Share
